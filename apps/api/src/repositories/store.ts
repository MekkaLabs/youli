...
export async function listInsights(userId: string): Promise<DailyInsight[]> {
  ...
  const { data, error } = await supabase.from('daily_insights').select('*').eq('profile_id', userId).order('created_at', { ascending: false }).limit(10);
...
export async function createInsight(userId: string, insight: DailyInsight): Promise<DailyInsight> {
  ...
  const payload = { id: withId.id, profile_id: userId, summary: withId.summary, actions: withId.actions, energy: withId.energy, created_at: withId.createdAt };
